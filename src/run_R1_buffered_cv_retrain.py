...
def run_spatial_cv(track_feats, bids, c_mask, y_wc, cv_folds, coords):
    print("=== Step 1: Running Source-Domain 3-Fold Spatial Cross-Validation (Seed 17) with 400m Buffer ===")
    ...
    for rep in REPS:
        for src in TR:
            ...
            for fd in fold_defs:
                ...
                # 400m Buffer implementation
                val_tree = cKDTree(coords[val_cand])
                dists, _ = val_tree.query(coords[tr_cand])
                tr_buffered = tr_cand[dists >= 400.0]
                
                rng = np.random.RandomState(17 + fd["fold"])
                tr_idx = rng.choice(tr_buffered, min(13333, len(tr_buffered)), replace=False)
                val_idx = rng.choice(val_cand, min(6667, len(val_cand)), replace=False)
                
                # Assertion
                assert cKDTree(coords[tr_idx]).query(coords[val_idx])[0].min() >= 400.0
    ...
    with open(WORKSPACE / "configs" / "R1_selected_models.json", "w", encoding="utf-8") as f:
        json.dump(selected_models, f, indent=2)
    
    return selected_models

def train_and_eval_final_models(track_feats, feature_names, bids, c_mask, y_wc, split_manifest, selected_models, coords):
    print("=== Step 2: Training & Evaluating Final Models with 400m Buffer ===")
    ...
    for seed in SEEDS:
        ...
        # Buffer main train vs all valid eval region (or eval_all_indices)
        ...
        # Assertion
        min_dist = cKDTree(coords[i_tr]).query(coords[eval_all_indices])[0].min()
        assert min_dist >= 400.0, f"Seed {seed}: min distance {min_dist} < 400m"
        ...
        for learner in ["RF", "SVC"]:
            for rep in REPS:
                for src in TR:
                    ...
                    run_dir = WORKSPACE / "runs_R1" / run_dir_name
    ...
    print("=== Step 3: Compute Comparisons ===")
    results = []
    ...
    for learner in ["RF", "SVC"]:
        for rep in REPS:
            for src in TR:
                for seed in SEEDS:
                    run_dir_name = f"{learner}_{rep}_{src}_s{seed}"
                    old_conf_path = WORKSPACE / "runs" / run_dir_name / "blocks_confusion.npy"
                    new_conf_path = WORKSPACE / "runs_R1" / run_dir_name / "blocks_confusion.npy"
                    
                    if old_conf_path.exists() and new_conf_path.exists():
                        old_cm = np.load(old_conf_path)
                        new_cm = np.load(new_conf_path)
                        
                        src_idx = TARGET_ORDER.index(src)
                        old_ba = compute_pooled_ba(old_cm[src_idx].sum(axis=0)) * 100.0
                        new_ba = compute_pooled_ba(new_cm[src_idx].sum(axis=0)) * 100.0
                        ...
                        results.append({
                            ...
                            "old_ba": old_ba,
                            "new_ba": new_ba,
                            "delta_ba": new_ba - old_ba
                        })
    
    df_comp = pd.DataFrame(results)
    tables_dir = WORKSPACE / "tables"
    tables_dir.mkdir(exist_ok=True)
    df_comp.to_csv(tables_dir / "R1_buffer_comparison.csv", index=False)
    
    mean_delta = df_comp["delta_ba"].mean()
    print(f"Mean Delta BA across all settings: {mean_delta:.2f}%")


def run_R1_buffered_cv_retrain():
    print("=== Starting R1: Buffered CV Retrain ===")
    ...
    selected_models = run_spatial_cv(track_feats, bids, c_mask, y_wc, cv_folds, coords)
    
    train_and_eval_final_models(track_feats, feature_names, bids, c_mask, y_wc, split_manifest, selected_models, coords)

if __name__ == "__main__":
    run_R1_buffered_cv_retrain()
